...
public class WonderTreeVar1Procedure {
			...
			if (!(airTag).isEmpty()) {
				...
				if (!dontSpawn && rotation == 0) {
					if (world instanceof ServerLevel _serverworld) {
						StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("silly_willy_core", "wondertree1"));
						if (template != null) {
